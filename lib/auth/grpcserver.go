...
func (g *GRPCServer) SendKeepAlives(stream proto.AuthService_SendKeepAlivesServer) error {
	...
	switch {
	case hasBuiltinRole(auth.Checker, string(teleport.RoleProxy)):
		serverKind = services.KindNode
	case hasBuiltinRole(auth.Checker, string(teleport.RoleNode)):
		serverKind = services.KindProxy
	default:
		return trail.ToGRPC(trace.AccessDenied("[10] access denied"))
	}
	clusterName, err := auth.GetDomainName()
	if err != nil {
		return trail.ToGRPC(err)
	}
	serverName, err := ExtractHostID(auth.User.GetName(), clusterName)
	if err != nil {
		log.Warn(err)
		return trail.ToGRPC(trace.AccessDenied("[10] access denied"))
	}
	...
	for {
		...
		if err := auth.KeepAliveNode(stream.Context(), *keepAlive); err != nil {
			return trail.ToGRPC(err)
		}
	}
}
